...
let CommentPlugin = class CommentPlugin extends components_1.ConverterComponent {
    ...
    moveCommentToSignatures(reflection, signatures) {
        ...
        if (reflection.name === 'getNodes')
            debugger;
        if (reflection.id === 3252 || reflection.id === 3657 || reflection.id === 3656)
            debugger;
            ...
            signature.parameters?.forEach((parameter, index) => {
                ...
                if (tag) {
                    if (!parameter.comment) {
                        parameter.comment = new models_1.Comment();
                    }
                    parameter.comment.summary = models_1.Comment.cloneDisplayParts(tag.content);
                }
                // move tags to types if possible: 
                // if(comment?.summary.filter(({tag}) => tag=== '@defaultValue' ).length === 1){
                // parameter.type.de
                // }
            });
